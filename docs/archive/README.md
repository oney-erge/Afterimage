...
accurate picture, see the three docs at `docs/` top level:

- **[HOW_IT_WORKS.md](../HOW_IT_WORKS.md)** — what the engine does, method by
  method, next to AirLLM. Start here.
...
(H1-H4) and the adaptive-speculation mechanisms (self-drafting, bandit-tuned
k, draft-aware VRAM planning). Every hypothesis in them was actually run;
the outcomes are in RESULTS_LOG.md and summarized in HOW_IT_WORKS.md. Useful
if you want the reasoning and kill-criteria behind a result, not just the
number.
