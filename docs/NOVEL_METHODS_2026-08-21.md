...
pageable RAM. Pinned H9 is therefore untested; pageable H9 is not supported by
the screen.
...
`replay_plan_state` generated by `afterimage optimize-residency`.
...
```bash
afterimage optimize-residency traces/calibration-*.json \
