...
if [ "$action" = repair ] || [ "$installed" != "$fingerprint" ] || [ ! -x "$exe" ]; then
  ...
  retry "PyTorch installation" "$uv" pip install --python .venv/bin/python "${reinstall[@]}" torch --index-url "$torch_index"
  retry "Afterimage installation" "$uv" pip install --python .venv/bin/python "${reinstall[@]}" --editable "$extras"
