...
| **5** | **H0** | Joint semantic/system oracle gap | Per-context oracle vs best global policy | L1 replay of fixed vs hazard speculation across four held-out families; **semantic bucket only** -- `system_bucket` is fixed at `cold_page_cache` for every observation, so no system-state variation is actually tested despite the hypothesis name | 0.13537 tok/s oracle | 0.13199 tok/s global | **+2.56% ceiling (semantic-only)** | N/A | N/A | Diagnostic; no memory change | Gate did its job for the semantic-only case: below 12%, so contextual/RL control has too little upside *under this fixed system-state regime*. This is an insufficient test of the registered joint semantic+system claim, not a falsification of it -- do not rerun (H0/H3/H8 are not decision-critical), just read the ceiling as semantic-only. |
