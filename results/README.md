...
[`INDEX.md`](INDEX.md) maps every file here to its hypothesis, status, and
which docs cite it -- generated by `scripts/build_results_index.py`, not
hand-maintained, so it cannot drift the way a hand-written index would.
Regenerate it after adding a result.
